		...
		<div className="custom-pagination">
			...
			<div className="table-nav">
				...
				<div className="page-select-nav">
					<select id="page-select-nav" value={currentPage} onChange={handlePageChange}
						className="border border-gray-300 rounded px-2 py-1">
						{pageItems.map((page) => (
				...
				<div className="next-prev-nav">
					<Button size="sm" variant="outline" onClick={()=> onChangePage(1, rowCount)}
						>
					...
					<Button size="sm" variant="outline" onClick={()=> onChangePage(Math.max(currentPage - 1, 1),
						rowCount)}
						>
						<svg className="rotate-90" xmlns="http://www.w3.org/2000/svg" width="30" height="30"
					...
					<Button size="sm" variant="outline" onClick={()=> onChangePage(Math.min(currentPage + 1,
						totalPages), rowCount)}
						>
						<svg className="-rotate-90" xmlns="http://www.w3.org/2000/svg" width="30" height="30"
							viewBox="0 0 30 30" fill="none">
					...
					<Button size="sm" variant="outline" onClick={()=> onChangePage(totalPages, rowCount)}
						>
...
const Table: React.FC<CustomTableProps<any>> = ({
	...
	typeof window !== 'undefined' && window.innerWidth <= 640 ); const handleResize=()=> {
	...
	const modifiedColumns = props.columns.map(column => ({
		...
		cell: (row: any) => (
			<>
				{(showExtraColumn && column.name !== "Action" && column.name !== "Aksi") && <div>
					{column.name}</div>}
	...
	return (
		<div className='table-container'>
			<div className="table-search">
				<InputSearch placeholder='Cari nama' setEnteredText={setFilterText}
					value={filterText} />
			</div>
			<DataTable 
				{...props} 
				...
				noDataComponent={<div className='p-6'> 
